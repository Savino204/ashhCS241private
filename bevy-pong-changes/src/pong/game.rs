...
fn goal(
    ...
    if !goal_event.is_empty() {
        ...
        for ev in goal_event.read() {
            match ev.0 {
                WhoScored::P1 => {
                    game_status.score.p1_score += 1;
                    //let mut f = File::open("player_num.txt").unwrap();
                    //let mut file_name = String::new();

                    //let _ = f.read_to_string(&mut file_name);
                    //let score = game_status.score.p1_score.to_string();
                    //fs::write(file_name, score).unwrap();
                }
